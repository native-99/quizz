                ...
                <div class="card-body">
                    ...
                    <h6>{{ $question_body }}</h2>
                        ...
                        @forelse($answers as $index => $answer)
                            @if (isset($workout) && $workout->WorkOutQuiz->where('question_id', $question->id)->first()->answer >= 0)
                                <div
                                    class=" p-2 
                    {{ $index == $correctAnswer ? ' border border-success text-success rounded font-weight-bold ' : '' }}
                    ...
                    ">
                                    <div class="form-check">
                                        <input name="answer" {{ $index == $correctAnswer ? 'checked' : '' }}
                                            class="form-check-input" type="radio" id="reviewAnswer{{ $index }}">
                            ...
                            @else
                                <div
                                    class=" p-2 {{ $index == $correctAnswer ? ' border border-success rounded font-weight-bold' : '' }}">
                                    <div class="form-check">
                                        <input name="answer" {{ $index == $correctAnswer ? 'checked' : '' }}
                                            class="form-check-input" type="radio"
                                            id="reviewAnswer{{ $index }}">
                                        <label class="form-check-label" for="reviewAnswer{{ $index }}">
                                            {{ $answer }}
                                        </label>
                                    </div>
