...
class FeedbackController extends Controller
{
    ...
    public function destroy(Feedback $feedback)
    {
        ...
        try {
            // Mencoba menghapus session
            $feedback->delete();
            return redirect()
                ->route("feedback.index")
                ->with('success', __('Item deleted successfully'));
        } catch (\Exception $e) {
            // Jika terjadi kesalahan, tampilkan pesan untuk menghapus relasi terkait terlebih dahulu
            return redirect()
                ->route("feedback.index")
                ->with('danger', __('Silahkan hapus isinya terlebih dahulu atau yang berkaitan di dalamnya.'));
        }
    }
