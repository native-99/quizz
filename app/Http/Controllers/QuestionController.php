...
class QuestionController extends Controller
{
    ...
    public function destroy(Question $question)
    {
        ...
        try {
            // Mencoba menghapus question
            $question->delete();
            return redirect()
                ->route("question.index")
                ->with('success', __('Question deleted successfully'));
        } catch (\Exception $e) {
            // Jika terjadi kesalahan (relasi terkait), tampilkan pesan kesalahan
            return redirect()
                ->route("question.index")
                ->with('danger', __('Delete failed. Please delete any related items or relationships first.'));
        }
    }
